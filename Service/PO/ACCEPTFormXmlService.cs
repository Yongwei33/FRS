...
{
    public class ACCEPTFormXmlService
    {
        public ACCEPTFormXmlService(string formVersionId, UrgentLevel Urgentlevel, string account, string userGuid, string userName, SW_ACCEPT_HEAD head, List<SW_ACCEPT_GRID> grid)
        {
        ...
        public string ConvertToFormInfoXml()
        {
            ...
            foreach (var Propertie in this.Fields.GetType().GetProperties())
            {
                    ...
                    default:
                        switch (field.FieldId)
                        {
                            ...
                            case "Date":
                                ...
                                break;
                            case "Dept":
                                FieldItemXE.Add(new XAttribute("fieldValue", head.Dept[0]));
                                FieldItemXE.Add(new XAttribute("realValue", $"<UserSet><Element type='group' isDepth='False'><groupId>" + head.Dept[3] + "</groupId></Element></UserSet>\r\n"));
                                break;
                            case "PODept":
                            ...
                            case "Applicant":
                                ...
                                break;
                            case "IsApplicant":
                                FieldItemXE.Add(new XAttribute("fieldValue", head.IsApplicant));
                                break;
                            /*case "ACCEPTDept":
                                FieldItemXE.Add(new XAttribute("fieldValue", head.ACCEPTDept[1]));
                                FieldItemXE.Add(new XAttribute("realValue", $"<UserSet><Element type='group' isDepth='False'><groupId>" + head.ACCEPTDept[0] + "</groupId></Element></UserSet>\r\n"));
                                break;
                            case "ACCEPTApplicant":
                                ...
                                break;*/
                            case "REQForm":
                                XElement REQFElement = new XElement("FormChooseInfo", new XAttribute("taskGuid", head.REQForm));
                                FieldItemXE.Add(REQFElement);
                                break;
                            case "POForm":
                                XElement POFElement = new XElement("FormChooseInfo", new XAttribute("taskGuid", head.POForm));
                                FieldItemXE.Add(POFElement);
                                break;
                            case "Vendor":
                                FieldItemXE.Add(new XAttribute("fieldValue", head.Vendor));
                                break;
                            case "TotalPrice":
                ...
                if (field.FieldId != "formNo" && field.FieldId != "Applicant" && field.FieldId != "TotalPrice" && field.FieldId != "Detail" && field.FieldValue != "")
                {
                    FieldItemXE.Add(new XAttribute("fillerName", this.UserName));
                    FieldItemXE.Add(new XAttribute("fillerUserGuid", this.UserGuid));
                    FieldItemXE.Add(new XAttribute("fillerAccount", this.Account));
    ...
    public class FieldList
    {
        ...
        public Field _AuditDeptField { get; set; } = new Field() { FieldType = FieldType.radioButton, FieldId = "AuditDept" };

        public Field _AdminiDeptField { get; set; } = new Field() { FieldType = FieldType.radioButton, FieldId = "AdminiDept" };

        public Field _MarketDeptField { get; set; } = new Field() { FieldType = FieldType.radioButton, FieldId = "MarketDept" };

        public Field _NewsDeptField { get; set; } = new Field() { FieldType = FieldType.radioButton, FieldId = "NewsDept" };

        public Field _FundDeptField { get; set; } = new Field() { FieldType = FieldType.radioButton, FieldId = "FundDept" };

        //public Field _ACCEPTApplicantField { get; set; } = new Field() { FieldType = FieldType.allDept, FieldId = "ACCEPTApplicant" };

        public OptionalField _REQFormField { get; set; } = new OptionalField() { FieldId = "REQForm" };

        public OptionalField _POFormField { get; set; } = new OptionalField() { FieldId = "POForm" };
        ...
        public Field _FileField { get; set; } = new Field() { FieldType = FieldType.multiLineText, FieldId = "File" };

        public Field _ACCEPTSignOffField { get; set; } = new Field() { FieldType = FieldType.allUser, FieldId = "ACCEPTSignOff" };

        public Field _PODeptField { get; set; } = new Field() { FieldType = FieldType.multiLineText, FieldId = "PODept" };

        public Field _IsApplicantField { get; set; } = new Field() { FieldType = FieldType.multiLineText, FieldId = "IsApplicant" };

        public Field _IsSignOffField { get; set; } = new Field() { FieldType = FieldType.multiLineText, FieldId = "IsSignOff" };
